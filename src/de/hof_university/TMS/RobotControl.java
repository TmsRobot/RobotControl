...
public class RobotControl {
    ...
    public void acknoledge(Command command) throws IOException {
        if (!command.getack1()) {
            if (command.getSource() == CONTROL) {
                Command ack = new Command(command.getDestination(), command.getSource(), 'A', 001, "");
                inout.sendMessage(control.getOutputStream(), ack);
                command.setack1();
            ...
            if (command.compare(CONTROL, CMD, "002")) {
                inout.sendMessage(control.getOutputStream(),  new Command(command.getDestination(), command.getSource(), 'A', 002, ""));
            }
